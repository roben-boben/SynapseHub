...
ssToggleLabel.Text = "Spawn Save Position"
...
-- ============================================================
-- 4. BACK TRAIL (ГОРИЗОНТАЛЬНЫЙ, НА СПИНЕ, ПЛАВНОЕ ИСЧЕЗНОВЕНИЕ)
-- ============================================================
pvY = pvY + itemHeight + gap
...
-- ЛОГИКА BACK TRAIL (ГОРИЗОНТАЛЬНЫЙ, НА СПИНЕ)
do
            ...
            while backTrailEnabled do
                ...
                if hrp and hum then
                    ...
                    if speed > 0.5 then
                        ...
                        stopTimer = stopTimer + 0.02
                        if stopTimer > 0.1 then
                            isMovingNow = false
                        end
                    end
                    
                    -- ПОЗИЦИЯ НА СПИНЕ (снаружи торса, на 0.5 студа назад)
                    local backPos = hrp.CFrame * CFrame.new(0, 0, -0.5)
                    local currentPos = backPos.Position
                    
                    -- ЕСЛИ ДВИЖЕТСЯ ИЛИ НЕДАВНО ДВИГАЛСЯ (до 1 секунды)
                    if isMovingNow or (tick() - lastMovingTime < 1.0) then
                        if lastPos then
                            local dist = (currentPos - lastPos).Magnitude
                            if dist > 0.15 then
                                -- СОЗДАЕМ ЧАСТЬ ТРЕЙЛА (горизонтальная полоска)
                                local part = Instance.new("Part")
                                part.Size = Vector3.new(0.15, 0.4, dist) -- УВЕЛИЧЕН ПО ВЕРТИКАЛИ В 1.5 РАЗА
                                ...
                                part.Color = Color3.fromRGB(colorValue, colorValue, colorValue)
                        ...
                        -- КОГДА ОСТАНОВИЛСЯ - ЛИНИЯ ПРОДОЛЖАЕТСЯ ДО КОНЦА
                        if #trailParts > 0 and lastPos then
                            -- ЖДЕМ ПОКА ВСЕ ЧАСТИ ДОЙДУТ ДО КОНЦА
                            local finalPos = lastPos
                            for i = #trailParts, 1, -1 do
                                local p = trailParts[i]
                                if p and p.Parent then
                                    -- ПЛАВНОЕ ИСЧЕЗНОВЕНИЕ
                                    local prog = i / #trailParts
                                    p.Transparency = 0.5 - (0.5 * (1 - prog))
                                    if prog < 0.05 then
                                        p:Destroy()
                ...
                -- ЕСЛИ ТРЕЙЛ ПУСТ - ВЫХОДИМ ИЗ ЦИКЛА
                if #trailParts == 0 and not isMovingNow and (tick() - lastMovingTime > 1.5) then
                    task.wait(0.1)
                end
